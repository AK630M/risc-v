...
#
#Arguments:
#          a0: pointer to RISC-V instructions
#          a1: pointer to WebAssembly code buffer
#Returns:
#          a0: size of WebAssembly code buffer
RISCVtoWASM:
# size_t RISCVtoWASM(unsigned *riscv, unsigned char* wasm) {
    # riscv = a0, wasm = a1
    mv      s4, a0                          # s4 <- a0 (RISC-V instructions)
    mv      s5, a1                          # s5 <- a1 (WebAssembly code buffer)
    # riscv = s4, wasm = s5

    # unsigned *riscv_copy = riscv;
    mv      s6, a0                          # s6 <- a0 (RISC-V instructions copy)

    #Generate target table for RISC-V instructions
    ...
    lw      a3, 0(s4)                       # load the first RISC-V instruction

    # check if (instr == 0xffffffff) goto main_loop_end Check if the instruction is end-of-program marker
    li      t0, 0xffffffff                  # 
...
main_loop:
    ...
    # instr = a0 -> opc = a0
    mv      a0, a3                          # extract the opcode of the RISC-V instruction
    jal     opcode
    mv      a2, a0
    # opc = a2
    
    #     instr >>= 4;
    srli    a3, a3, 4                       #Shift the RISC-V instruction right to discard the opcode

    #     unsigned type = instr & 0b111;
    andi    a5, a3, 0x7                     #Extract the type of the RISC-V instruction

    #     unsigned cnt = forwardCount[(size_t)(riscv - riscv_copy)];
    sub     t0, s4, s6
    srli    t0, t0, 2
    lb      t0, %lo(forwardCount)(t0)       ## t0 <- forwardCount[t0]

    j       loop_forward_check              #jump to loop_forward_check
loop_forward_begin:
    #         *p = 0x0b;
    li      t1, 0x0b                        # 0x0b = end opcode
    sb      t1, 0(a4)                       # store end opcode in WebAssembly code buffer

    #         p += 1;
    addi    a4, a4, 1                       # increment the pointer WebAssembly code buffer

    #         cnt--;
    addi	t0, t0, -1                          # decrement forwardCount[t0]
    
loop_forward_check:
    #     while (cnt > 0) {
    bgt		t0, zero, loop_forward_begin    # loop while forwardCount[t0] > 0
...
loop_forward_end:
    ...
    sub     t0, s4, s6
    srli    t0, t0, 2
    lb      t0, %lo(backwardCount)(t0)      # t0 <- backwardCount[t0]
...
loop_backward_check:
    #     while (cnt > 0);
    bgt     t0, zero, loop_backward_begin   # loop while backwardCount[t0] > 0
...
main_loop_end:
    ...
    # p += 4;
    addi    a0, a0, 4           # Increment the program counter "p" by 4.
